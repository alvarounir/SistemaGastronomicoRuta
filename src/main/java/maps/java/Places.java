...
public class Places extends MapsJava {
    ...
    private URL createURL(double latitude, double longitude,int radius,String keyword,String namePlace,
            Rankby rankby,String types,Boolean opennow) throws UnsupportedEncodingException, MalformedURLException{
        ...
        if(types!=null && !types.isEmpty()){
            _types="&types=" + URLEncoder.encode(types, "utf-8");
        }
        String _opennow="";
        if(opennow){
            _opennow= "&opennow=" + opennow;
        }
        URL urlReturn=new URL(_location + _radius + _keyword + _namePlace + _rankby +
                _types + _opennow + super.getSelectPropertiesRequest() + "&key=" + MapsJava.getKey());
        return urlReturn;
    }
    ...
    public String[][] getPlaces(double latitude, double longitude,int radius,String keyword,String namePlace,
            Rankby rankby,String types, Boolean opennow) throws UnsupportedEncodingException, MalformedURLException{
        URL url=createURL(latitude,longitude,radius,keyword,namePlace,rankby,types,opennow);
        try {
            ...
            NodeList nodeReference = (NodeList) xpath.evaluate("PlaceSearchResponse/result/reference", 
                         document, XPathConstants.NODESET);
            NodeList nodeRating = (NodeList) xpath.evaluate("PlaceSearchResponse/result/rating", 
                    document, XPathConstants.NODESET);
            NodeList nodeType = (NodeList)  xpath.evaluate("PlaceSearchResponse/result/type", 
                    document, XPathConstants.NODESET);
            NodeList nodePlaceId = (NodeList)  xpath.evaluate("PlaceSearchResponse/result/place_id", 
                    document, XPathConstants.NODESET);
            ...
            allNodes.add(nodeRating);allNodes.add(nodeType);allNodes.add(nodePlaceId); 
